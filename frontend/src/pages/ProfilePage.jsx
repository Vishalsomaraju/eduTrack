...
// Fields that belong to each endpoint — we only send relevant fields,
// never the whole profile object (avoids sending id, role, email etc.)
...
const SECTION_ALLOWED_FIELDS = {
  ...
  academic: STUDENT_FIELDS,
  family: STUDENT_FIELDS,
  professional: FACULTY_FIELDS,
};
...
export default function ProfilePage() {
  ...
  const handleEdit = (sectionKey) => {
    ...
    // Only copy fields relevant to this section (avoids polluting formData
    // with id/role/email/created_at which would confuse the backend)
    const source =
      sectionKey === "personal" ? data.profile : (data.detail ?? {});
    const allowed = SECTION_ALLOWED_FIELDS[sectionKey] ?? [];
    const initial = {};
    allowed.forEach((k) => {
      initial[k] = source[k] ?? null;
    });
    setFormData(initial);
  };
  ...
  const handleSave = async (sectionKey, endpoint) => {
    setSaving(true);
    try {
      // Convert empty strings → null so the backend's exclude_none removes them
      // (Pydantic raises 422 if it receives "" for a date/int field)
      ...
      // Update local state so the view reflects the change immediately
      const updatedData = { ...data };
      if (sectionKey === "personal") {
        updatedData.profile = { ...data.profile, ...payload };
      } else {
        updatedData.detail = { ...(data.detail ?? {}), ...payload };
      }
      setData(updatedData);
      setEditing(null);
  ...
  const handleAvatarUpload = async (e) => {
    ...
    try {
      ...
      setData((p) => ({
        ...p,
        profile: { ...p.profile, avatar_url: responseData.avatar_url },
      }));
  ...
  const initials = !profile.name
    ...
    : profile.name.trim().split(/\s+/).length === 1
      ...
      : (
          ...
          profile.name.trim().split(/\s+/).slice(-1)[0][0]
        ).toUpperCase();

  const formatAdmissionType = (type) =>
    ({
      convenor: "Convenor Quota",
      ...
      >
        ...
        >
          ...
          <div
            style={{
              ...
              marginTop: 12,
              border: `1px solid ${role === "student" ? "var(--accent-green-border)" : role === "admin" ? "var(--accent-blue-border)" : "var(--border)"}`,
            }}
          >
            {role}
          </div>
          ...
          {role === "student" && (
            ...
            >
              {[
                "📅 Year " + (detail?.year || "—"),
                "📚 Sem " + (detail?.semester || "—"),
                "🎓 " + formatAdmissionType(detail?.admission_type),
              ].map((txt, i) => (
                <div
                  key={i}
                  style={{
        ...
        <div>
          ...
          <SectionCard
            ...
            onSave={() => handleSave("personal", "/auth/profile/me")}
          >
            <div
              style={{
                display: "grid",
              ...
              {editing === "personal" ? (
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setFormData({
                          ...formData,
                          blood_group: e.target.value || null,
                        })
                      }
                      style={inputStyle}
                    >
                ...
                <>
                  ...
                  <Field
                    ...
                    value={
                      profile.aadhar
                        ? `XXXX-XXXX-${profile.aadhar.slice(-4)}`
                        : null
                    }
                  />
          ...
          {role === "student" && (
            <SectionCard
              ...
              onCancel={handleCancel}
              onSave={() => handleSave("academic", "/auth/profile/me/student")}
            >
              <div
                style={{
                  display: "grid",
                ...
                {editing === "academic" ? (
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setFormData({
                            ...formData,
                            admission_type: e.target.value || null,
                          })
                        }
                        style={inputStyle}
                      >
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setFormData({
                            ...formData,
                            joined_date: e.target.value || null,
                          })
                        }
                        style={inputStyle}
                      />
                  ...
                  <>
                    ...
                    <Field
                      ...
                      value={
                        detail?.year
                          ? `${detail.year}${detail.year === 1 ? "st" : detail.year === 2 ? "nd" : detail.year === 3 ? "rd" : "th"} Year`
                          : null
                      }
                    />
                    ...
                    <Field
                      ...
                      value={formatAdmissionType(detail?.admission_type)}
                    />
          ...
          {role === "student" && (
            <SectionCard
              ...
              onCancel={handleCancel}
              onSave={() => handleSave("family", "/auth/profile/me/student")}
            >
              {editing === "family" ? (
                <div
                  style={{
          ...
          {/* Professional Details (faculty/admin) */}
          {(role === "faculty" || role === "admin") && (
            <SectionCard
              ...
              onCancel={handleCancel}
              onSave={() =>
                handleSave("professional", "/auth/profile/me/faculty")
              }
            >
              <div
                style={{
